...
$is_short_url = request_var('surl', false);
...
// ---------- BEGIN [DMMO] EOS MODIFICATION ----------
$parent_id = (int) $topic_data['parent_id'];
...
// ---------- BEGIN [DMMO] EOS MODIFICATION ----------
// If we've got a hightlight set pass it on to pagination. Custom function in includes/functions.php
$pagination = dmmo_generate_pagination(append_sid("{$phpbb_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id" . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : '') . (($highlight_match) ? "&amp;hilit=$highlight" : '')), $total_posts, $config['posts_per_page'], $start);
// ---------- END [DMMO] EOS MODIFICATION ------------
...
$template->assign_vars(array(
	...
	'TOPIC_ID' 		        => $topic_id,
    // ---------- BEGIN [DMMO] EOS MODIFICATION ----------
	'DMMO_TOPIC_TITLE' 	        => getShortenedText($topic_data['topic_title'], 60),
    'DMMO_TOPIC_TITLE_SHORTER'   => getShortenedText($topic_data['topic_title'], 32),
    // ---------- END [DMMO] EOS MODIFICATION ------------
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
    ...
    // ---------- BEGIN [DMMO] EOS MODIFICATION ----------
    $admins = $auth->acl_get_list(false, 'a_', false);
    $mods = $auth->acl_get_list(false, 'm_', false);

    $is_admin = in_array($poster_id, $admins[0]['a_']);
    if(!$is_admin)
       $is_mod = in_array($poster_id, $mods[0]['m_']);
	...
	$postrow = array(
		...
		'POST_DATE_REL'		=> getRelativeTime($row['post_time']),
		'POST_SUBJECT'		=> $row['post_subject'],
		'MESSAGE'			=> $message,
