...
// ---------- BEGIN [DMMO] EOS MODIFICATION ----------
$parent_id	= (!empty($post_data['parent_id'])) ? (int) $post_data['parent_id'] : (int) $forum_id;

// Query for parent forum's name (i.e. the filter's parent category name).
$sql = 'SELECT forum_name FROM ' . FORUMS_TABLE . ' WHERE forum_id = ' . $parent_id;
$result = $db->sql_query($sql);
$parent_name = $db->sql_fetchrow($result);
$parent_name = $parent_name['forum_name'];
// ---------- END [DMMO] EOS MODIFICATION ------------
...
// Treat authorization differently for post creation.
if(empty($submit) && empty($preview) && empty($save)) {

    // Authorize post creation for categories.
    if ($post_data['forum_type'] != FORUM_CAT && $post_data['forum_type'] != FORUM_POST && in_array($mode, array('post', 'bump', 'quote', 'reply')))
    {
        trigger_error('USER_CANNOT_FORUM_POST');
    }
    
    // Ensure a postable forum exists within category.
    elseif ($post_data['forum_type'] == FORUM_CAT) {
    
        
    
    }
    
// Default PHPBB authorization otherwise.    
} else {
...
$template->assign_vars(array(
    ...
    'MODE'                  => $mode,
    'S_FIRST_POST'          => ($post_data['topic_first_post_id'] == $post_data['post_id']) ? true : false,
	'DMMO_U_PARENT'         => append_sid("{$phpbb_root_path}viewforum.$phpEx", "f=$parent_id"),
	'DMMO_U_FORUM_FILTERED' => append_sid("{$phpbb_root_path}viewforum.$phpEx", "f=$parent_id&amp;efid[]=$forum_id"),
    // ---------- END [DMMO] EOS MODIFICATION ------------
...
if($mode == 'post') {
    ...
    // If the forum is not a category, get its parent.
    if($post_data['forum_type'] != FORUM_CAT) {

        // DisneyMMO EOS does not permit nested subforums. If it did, this would be problematic.
        $sql = 'SELECT * FROM ' . FORUMS_TABLE . " where forum_id = {$post_data['parent_id']}";
        $result = $db->sql_query($sql);
        $forum_data = $db->sql_fetchrow($result);
        
    // Store data for forum for future use below.    
    } else {

        $forum_data = $post_data;
    }

    // Gather the filters for the forum category in question. This category will either be one
    // chosen by the user explicitly, or it will be the parent of a sub-forum chosen by the user.
...
$template->assign_var('TESTING_PHASE', true);
