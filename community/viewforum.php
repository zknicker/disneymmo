...
*
* Extensively modified for DisneyMMO EOS filters modification. Changes are indicated by the [DMMO] label, which you can search for.
*/
...
// [DMMO] Get the effective forum id's array from client.
$effective_forum_ids = request_var('efid', array('' => 0));
...
// Use modified generate_forum_nav function for added functionality.
dmmo_generate_forum_nav($forum_data);
...
$is_filter_category = false;
if ($forum_data['left_id'] != $forum_data['right_id'] - 1)
{
    $is_filter_category = true;
    // ---------- BEGIN [DMMO] EOS FILTERS MODIFICATION ----------
    
    // The display_forums call has been replaced with a dmmo_display_forums call, which takes into consideration
    // effective_forum_ids, as well as sanitizes said list of effective_forum_ids.
	list($active_forum_ary, $moderators, $filtered_forum_ary) = dmmo_display_forums($forum_data, $config['load_moderators'], $config['load_moderators'], $effective_forum_ids);
// ---------- END [DMMO] EOS FILTERS MODIFICATION ------------
}
else
{
...
// Retrieve array representation of discussion filters indexed with forum id.
// This is used later when displaying topics, to get their filter icons and info.
$sql = 'SELECT * FROM phpbb_discussion_filters';
$result = $db->sql_query($sql);
$discussion_filters = array();

while($dfrow = $db->sql_fetchrow($result)) {
...
// category's forums strictly because for the purpose of the filter modification,
// because the array 'forum_id' has been hijacked in the dmmo_display_forums function
// to be populated only with the forum ids corresponding to topics from forums
// currently being displayed per the user's filter selections.
//
// However, we make sure to light up, 'make active', etc. those filters which are
// currently filtered by the user by making use of 'forum_id' array, which again,
// contains the ids of forums which are a part of the user's filter selection.

// $are_filtered is true when the user has selected filters.
if($is_filter_category && sizeof($active_forum_ary)) {

    $are_filtered = !empty($filtered_forum_ary['effective_forum_ids']);
    $template->assign_vars(array(
        'FILTERS_ARE_ACTIVE'	=> $are_filtered,
    ));
    foreach($filtered_forum_ary['forum_id_all'] as $fid) {
        
        // $filter_active is true when this specific filter (aka forum id) is active, or selected by the user.
...
// Assign the page id for footer js including logic. This is unrelated to the above.
$template->assign_vars(array(
    'PAGE_ID'   => 'discussions_list'
));
// ---------- END [DMMO] EOS FILTERS MODIFICATION ------------
...
if ($mark_read == 'topics')
{
	...
	{
        // ---------- BEGIN [DMMO] EOS FILTERS MODIFICATION ----------
        if($is_filter_category) {
        
            // Categories (DMMO implementation).
            // Add forum IDs of subforums to array_merge function (for categories).
            markread('topics', array_merge($active_forum_ary['forum_id'], array($forum_id, 0)));
        
        } else {
        
            // Regular forums (PHPBB implementation).
...
// ---------- BEGIN [DMMO] EOS FILTERS MODIFICATION ----------
// Set proper topic count for categories (topic count not ordinarily displayed for categories).
elseif ($is_filter_category)
{
    $topics_count = $active_forum_ary['forum_topics'];
    $sql_limit_time = '';
}
else
{
...
$template->assign_vars(array(
    ...
    // ---------- BEGIN [DMMO] EOS FILTERS MODIFICATION ----------
    // Make true when forum type is a category.
	'S_DISPLAY_POST_INFO'	=> (($forum_data['forum_type'] == FORUM_POST || $forum_data['forum_type'] == FORUM_CAT) && ($auth->acl_get('f_post', $forum_id) || $user->data['user_id'] == ANONYMOUS)) ? true : false,
    // ---------- END [DMMO] EOS FILTERS MODIFICATION ------------
...
{
    // ---------- BEGIN [DMMO] EOS FILTERS MODIFICATION ----------
    // Join the forums table and select the forum name for each topic id being displayed.
    $sql_array['SELECT'] .= ', f.forum_name';
    $sql_array['LEFT_JOIN'][] = array('FROM' => array(FORUMS_TABLE => 'f'), 'ON' => 'f.forum_id = t.forum_id');
    
    // Join the users table and select some user information for each topic id being displayed.
    $sql_array['SELECT'] .= ', u.user_id, u.user_avatar, u.user_avatar_type';
...
if (sizeof($shadow_topic_list))
{
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		$row = array_merge($row, array(
            ...
            // ---------- BEGIN [DMMO] EOS FILTERS MODIFICATION ----------
            // Post values obtained earlier to template vars.
            'forum_name'        => $rowset[$orig_topic_id]['forum_name'],
            'user_id'           => $rowset[$orig_topic_id]['user_id'],
            'user_avatar'       => $rowset[$orig_topic_id]['user_avatar'],
            'user_avatar_type'  => $rowset[$orig_topic_id]['user_avatar_type']
            // ---------- END [DMMO] EOS FILTERS MODIFICATION ------------
		));
...
{
// ---------- BEGIN [DMMO] EOS FILTERS MODIFICATION ----------
// Don't kill the topic count (fix: comment it out)!
// $topics_count = 1;
// ---------- END [DMMO] EOS FILTERS MODIFICATION ------------
}
...
// ---------- BEGIN [DMMO] EOS FILTERS MODIFICATION ----------
// These are extra template vars that have been added, or otherwise modified for DMMO EOS.
$template->assign_vars(array(
    // Change pagination to conform with different html.
	'PAGINATION'	=> dmmo_generate_pagination(append_sid("{$phpbb_root_path}viewforum.$phpEx", "f=$forum_id" . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : '')), $topics_count, $config['topics_per_page'], $start, false, '', $filtered_forum_ary['effective_forum_ids']),
	'PAGE_NUMBER'	=> on_page($topics_count, $config['topics_per_page'], $start),
    
    // Remove the condition that prevents total_topics display on the category (active_topics) page.
	'TOTAL_TOPICS'	=> (($total_topic_count == 1) ? $user->lang['VIEW_FORUM_TOPIC'] : sprintf($user->lang['VIEW_FORUM_TOPICS'], $total_topic_count)),
    
    // Set up the page's search to search only topics in the category.
    'S_SEARCH_HIDDEN_FIELDS' => '<input type="hidden" name="fid[0]" value="' . $forum_id . '" />')
...
if (sizeof($topic_list))
{
	...
	foreach ($topic_list as $topic_id)
	{
		...
		$template->assign_block_vars('topicrow', array(
            ...
            // ---------- BEGIN [DMMO] EOS FILTERS MODIFICATION ----------
            'LAST_POST_TIME_RELATIVE'	=> getRelativeTime($row['topic_last_post_time']), /* [DMMO] */
