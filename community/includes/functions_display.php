...
/**
* A DisneyMMO [DMMO] branded modification of the display_forums function. Additional functionality includes that of
* an extra parameter which takes an array of forum ids which correspond to a subset of the category's sub-forum ids
* that are to actually be displayed to the user. Those forum ids not present in this array are to be completely ignored
* when constructing this category's 'display forums' and associated data. If the array is empty, then the function should
* proceed as it normally would in its unmodified state.
*
* Note that this function also disregards forum ids which do not belong to the category, as well as the case where a forum
* id provided corresponds to the id of the category itself.
*/
function dmmo_display_forums($root_data = '', $display_moderators = true, $return_moderators = false, $effective_forum_ids = array())
{
	global $db, $auth, $user, $template;
	global $phpbb_root_path, $phpEx, $config;

    $forum_rows = $subforums = $forum_ids = $forum_ids_moderator = $forum_moderators = $active_forum_ary = /* [DMMO] */ $filtered_forum_ary /* [END DMMO] */= array();
    ...
    // We are going to hijack the SQL query for our own use.
    while ($row = $db->sql_fetchrow($result))
    {
        // Preserve row to give to the rest of the function later on, since we hijacked this SQL query's results.
        $rows[] = $row;
        
        // Assemble a list of all of the forum_ids for use in sanitizing the effective_forum_ids array.
        $forum_ids[] = $row['forum_id'];
    }
    
    // Intersect the effective_forum_id array with that of the forum_ids array, such that only valid forum_ids are kept.
    $filtered_forum_ary['effective_forum_ids'] = array_intersect($effective_forum_ids, $forum_ids);
    
    // Now restore control to the function, using our rows array that we constructed.
	...
	foreach($rows as $row)
	{
        ...
        // Don't proceed with this forum_id if the user doesn't have permission to the forum.
        if (!$auth->acl_get('f_list', $forum_id))
        {
            continue;
        }
        
        // Check to see if the current forum_id being processed is a member of the user's effective forum_ids.
        $is_active = in_array($forum_id, $filtered_forum_ary['effective_forum_ids']) ? 1 : 0;
        
        // Regardless, we need to record the forum_id as a member of this category for the filter menu display,
        // and for later generating the list of discussions for the category.
        $filtered_forum_ary['forum_id_all'][] = $forum_id;
        
        // We also need the filter names (i.e. forum names) regardless (indexed by forum_id).
        $filtered_forum_ary['forum_id_all_name'][$forum_id] = $row['forum_name'];
        
        // If the current forum_id is not in user's list of effective_forum_ids, then skip this loop itteration.
        if(!$is_active && !empty($filtered_forum_ary['effective_forum_ids'])) {
        
            // This forum id is not part of the filter, so move on.
            continue;
	...
	if ($return_moderators)
	{
		return array($active_forum_ary, $forum_moderators, $filtered_forum_ary);
	}

	return array($active_forum_ary, array(), $filtered_forum_ary);
}
...
/**
* [DMMO] Additional functionality and modifications to the
* PHPBB generate_forum_nav function.
*/
function dmmo_generate_forum_nav(&$forum_data)
{
	global $db, $user, $template, $auth, $config;
	global $phpEx, $phpbb_root_path;
    
    // Include the eos files if needed.
    include_once($phpbb_root_path . '../php/eos.functions.php');
    include_once($phpbb_root_path . '../php/eos.config.php');
    ...
    $forum_meta = getForumMeta($forum_data['forum_id']);
    
	$template->assign_vars(array(
		'FORUM_ID' 		=> $forum_data['forum_id'],
		'FORUM_ABBR' 	=> $forum_meta['abbr'],
		'FORUM_NAME'	=> $forum_data['forum_name'],
		'FORUM_DESC'	=> generate_text_for_display($forum_data['forum_desc'], $forum_data['forum_desc_uid'], $forum_data['forum_desc_bitfield'], $forum_data['forum_desc_options']),
...
function get_user_avatar($avatar, $avatar_type, $avatar_width, $avatar_height, $alt = 'USER_AVATAR', $ignore_config = false)
{
	...
	{
		$avatar_img = 'http://disneymmo.com/images/layout/default/avatar-default.png';
