...
function compose_pm($id, $mode, $action, $user_folders = array())
{
	...
	if ($action != 'edit')
	{
		...
		if ($config['allow_mass_pm'] && $auth->acl_get('u_masspm_group'))
		{
			...
			while ($row = $db->sql_fetchrow($result))
			{
                ...
                // Assign block vars for group options for the mod.
                $template->assign_block_vars('group_option', array(
					'GROUP_NAME'	=> ($row['group_type'] == GROUP_SPECIAL) ? $user->lang['G_' . $row['group_name']] : $row['group_name'],
                    'GROUP_ID'      => $row['group_id'],
                    'IS_SPECIAL'    => $row['group_type'] == GROUP_SPECIAL)
				);
            }
    ...
    // Retrieve the mod recipient inputs.
	$u_to = request_var('u_recipients_to', array(0 => ''));
	$g_to = request_var('g_recipients_to', array(0));
	$u_bcc = request_var('u_recipients_bcc', array(0 => ''));
	$g_bcc = request_var('g_recipients_bcc', array(0));
    
    if (!$u_to && !$g_to && !$u_bcc && !$g_bcc)
    {
        // Handle User/Group adding/removing if any pre-mod inputs are in use.
        // This is for backwards compatability with unmodded templates.
        handle_message_list_actions($address_list, $error, $remove_u, $remove_g, $add_to, $add_bcc);
    }
    else 
    {
        // Handle the user/group adding/remove for the post-mod inputs.
        handle_recipients_actions($address_list, $error, $u_to, $g_to, $u_bcc, $g_bcc);
    }
    // ---------- END [DMMO] EOS MODIFICATION ------------
	...
	$template->assign_vars(array(
        ...
        // ---------- BEGIN [DMMO] EOS MODIFICATION ----------
        'USERNAME_AUTOCOMPLETE_URL' => $phpbb_root_path . '../ajax/autocomplete_username.php',
        // ---------- END [DMMO] EOS MODIFICATION ------------
	));
...
/**
* [DMMO] A modification on the handle_message_list_actions which coordinates
* construction of the address_list array for the PM recipients mod.
*
* This is only modified as much as necessary from the original function.
*/
function handle_recipients_actions(&$address_list, &$error, $u_to, $g_to, $u_bcc, $g_bcc)
{
	global $auth, $db, $user;

	// Unset all users and groups. They will be readded if they are still
    // in the users recipient selections.
    unset($address_list['u']);
    unset($address_list['g']);
    
	// Add users and groups for each of 'to' and 'bcc'
    $types = array('to', 'bcc');
	foreach($types as $type)
	{
        $u_recipients = ($type == 'to') ? $u_to : $u_bcc;
        $g_recipients = ($type == 'to') ? $g_to : $g_bcc;
    
        // Add group recipients.
		if (sizeof($g_recipients))
		{
			foreach ($g_recipients as $group_id)
			{
				$address_list['g'][$group_id] = $type;
			}
		}
		...
		// Reveal the correct user_ids
		if (sizeof($u_recipients))
		{
			user_get_id_name($user_id_ary, $u_recipients, array(USER_NORMAL, USER_FOUNDER, USER_INACTIVE));
