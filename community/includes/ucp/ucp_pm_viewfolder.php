		...
		if (sizeof($folder_info['pm_list']))
		{
			...
			foreach ($folder_info['pm_list'] as $message_id)
			{
				...
				$template->assign_block_vars('messagerow', array(
                    ...
                    'MESSAGE_AUTHOR_AVATAR_URL' => dmmo_get_user_avatar_path($row['user_avatar'], $row['user_avatar_type']),
                    'MESSAGE_AUTHOR_LINK'       => append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=viewprofile&u=' . $row['author_id']),
                    'SENT_TIME_REL'             => getRelativeTime($row['message_time']),
...
function get_pm_from($folder_id, $folder, $user_id)
{
	...
	$template->assign_vars(array(
        // ---------- BEGIN [DMMO] EOS MODIFICATION ----------
        // Use DMMO pagination.
		'PAGINATION'		=> dmmo_generate_pagination(append_sid("{$phpbb_root_path}ucp.$phpEx", "i=pm&amp;mode=view&amp;action=view_folder&amp;f=$folder_id&amp;$u_sort_param"), $pm_count, $config['topics_per_page'], $start),
        // ---------- END [DMMO] EOS MODIFICATION ------------
    ...
    // ---------- BEGIN [DMMO] EOS MODIFICATION ----------
    // Modify SQL to also receive the user's avatar.
    $sql = 'SELECT t.*, p.root_level, p.message_time, p.message_subject, p.icon_id, p.to_address, p.message_attachment, p.bcc_address, u.username, u.username_clean, u.user_colour, u.user_avatar, u.user_avatar_type, p.message_reported
		FROM ' . PRIVMSGS_TO_TABLE . ' t, ' . PRIVMSGS_TABLE . ' p, ' . USERS_TABLE . " u
