...
class ucp_zebra
{
	...
	function main($id, $mode)
	{
		...
		if ($submit)
		{
			...
			{
				if (confirm_box(true) || true) // [DMMO] Just remove the confirmation box...
				{
					// Remove users
        ...
        // Add SQL select for user avatar and the avatar type.
		...
		while ($row = $db->sql_fetchrow($result))
		{
            ...
            // ---------- BEGIN [DMMO] EOS MODIFICATION ----------
            // Add support for username options as separate hidden inputs.
            $template->assign_block_vars('username_options', array(
                'USER_ID'   => $row['zebra_id'],
                'USERNAME'  => $row['username'],
                'AVATAR'    => dmmo_get_user_avatar_path($row['user_avatar'], $row['user_avatar_type']))
            );
            // ---------- END [DMMO] EOS MODIFICATION ------------
		}
		$db->sql_freeresult($result);
		...
		$template->assign_vars(array(
            ...
            // ---------- BEGIN [DMMO] EOS MODIFICATION ----------
            'FRIENDS_REMOVE_AJAX_URL' => $phpbb_root_path . '../ajax/friends_remove.php')
            // ---------- END [DMMO] EOS MODIFICATION ------------
        );
