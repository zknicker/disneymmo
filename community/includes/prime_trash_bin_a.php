	...
	function get_stifled_topic_msg(&$topic_data)
	{
		...
		if($can_view_delete && !empty($topic_data['post_text']))
		{
			$template_vars = array_merge($template_vars, array(
				...
				'VIEW_LINK'	=> '<a href="#view_post" class="view_deleted_post" onclick="return(show_post(this,\'' . $deleted_post_id . '\'));">View Post</a>',
			));
		}
	...
	*/
	function get_stifled_post_msg(&$post_data, $message = '', $title = '')
	{
		...
		$template_vars = array(
			...
			'UNDO_LINK'		=> $can_undo_delete ? '<a href="' . append_sid("{$phpbb_root_path}$undo_delete_page", $undo_delete_args . $redirect) . '" class="post_undelete">Undelete</a>' : '',
			'PERM_LINK'		=> $can_perm_delete ? '<a href="' . append_sid("{$phpbb_root_path}$perm_delete_page", $perm_delete_args . $redirect) . '" class="post_delete_forever">Permanently Delete</a>' : '',
			'MINI_POST_IMG'	=> $user->img('icon_post_target', $user->lang['POST']),
		...
		if($can_view_delete && !empty($message))
		{
			$template_vars = array_merge($template_vars, array(
				...
				'VIEW_LINK'	=> '<a href="#view_post" class="view_deleted_post" onclick="return(show_post(this,\'' . $deleted_post_id . '\'));">View Original</a>',
			));
		}
		$template->set_filenames(array('delete_msg'	=> 'prime_deleted_msg.html'));
		$template->assign_block_vars('deleted', $template_vars);
