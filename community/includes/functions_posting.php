...
/**
* [DMMO] Assign all smilies with appropriate data to a template variable for reference by CKEditor.
*/
function dmmo_generate_ckeditor_smilies()
{
    ...
    foreach ($cats as $cat) {
        ...
        // Store category name and smiley info in an array.
        while ($row = $db->sql_fetchrow($result))
        {        
            if (empty($smilies[$row['smiley_url']]))
            {
                $smilies[$cat_name][$row['smiley_url']] = $row;
            }
        }
	...
	if (sizeof($smilies))
	{
 		foreach ($smilies as $cat_name => $cat)
		{
            foreach ($cat as $row)
            {
                $template->assign_block_vars('smilies_images', array(
                    'SMILEY_REL_URL' => $row['smiley_url'])
                );
                $template->assign_block_vars('smilies_desc', array(
                    'SMILEY_CODE'	 => $row['code'])
                );
                
                $template->assign_block_vars('smilies_cat', array(
                    'SMILEY_CAT'	 => $cat_name)
                );
            }
		}
...
function topic_review($topic_id, $forum_id, $mode = 'topic_review', $cur_post_id = 0, $show_quote_button = true)
{
	...
	$sql = $db->sql_build_query('SELECT', array(
        ...
        // [DMMO] Added avatar fields from users table to SELECT.
		'SELECT'	=> 'u.username, u.user_id, u.user_colour, u.user_avatar, u.user_avatar_type, p.*, z.friend, z.foe',
...
//-- end: Prime Trash Bin (Posts) -------------------------------------------//
	...
	for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
	{
		...
		$template->assign_block_vars($mode . '_row', array(
            ...
            'POST_AUTHOR_AVATAR'    => get_user_avatar($row['user_avatar'], $row['user_avatar_type'], '100%', '100%'), // [DMMO]
            ...
            'POST_DATE_REL'     => getRelativeTime($row['post_time']), // [DMMO]
			'MESSAGE'			=> $message,
...
function submit_post($mode, $subject, $username, $topic_type, &$poll, &$data, $update_message = true, $update_search_index = true)
{
    ...
    // If the topic is being posted as an article, create an article entry.
    if($mode == 'post' || $mode == 'edit') {
    
        include_once $phpbb_root_path . '../php/eos.config.php';
        include_once $phpbb_root_path . '../php/eos.functions.php';
        
        // Verify that the topic is being posted as an article.
        $posts_articles = isForumAnArticlePoster($data['forum_id']);
        
        if($posts_articles) {
        
            $articles_thumbnail = $data['article_thumbnail'];
            $articles_preview = $data['article_preview'];
            if(strlen($articles_preview) == 0) {
            
                $articles_preview = $data['message'];
            }
            
            createArticleEntry($data['topic_id'], $data['forum_id'], $articles_thumbnail, $articles_preview);
        }
    }
